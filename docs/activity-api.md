...
Withdrawal photo lifecycle (2026-09-21): photos remain during the seven-day cancellation window. At final account purge, currently attached review files are deleted from storage and `photo_url` becomes null; review text/rating remain under the detached actor. Storage failures keep the account/references available for the next purge attempt. Storage deletion cannot roll back with a database transaction. Ordinary review deletion/photo replacement still does not clean old files; existing orphaned files and previously purged accounts are a separate remediation task. Production must schedule and monitor `purge_withdrawn_users`.
