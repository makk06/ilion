...
> **2026-09-21 갱신:** 아래 번호 목록은 과거 조사 이력입니다. 최신 출시 범위와 잔여 작업은 [법적 문서 준비 상태](../legal/README.md) 및 [배포 전 작업 목록](../release-readiness-2026-09-21.md)을 우선합니다. 위치는 제거 대신 비활성화했으며, 탈퇴 최종 파기 시 현재 첨부 사진 삭제를 구현했습니다. 일반 후기 삭제/교체의 파일 정리는 여전히 미완료입니다.
...
## 탈퇴 기능 구현으로 새로 생긴 잔여 위험 (2026-09-18)

회원 탈퇴 기능을 구현하면서 **알고 수용한** 위험이다. 몰라서 남은 것이 아니므로 별도 과제로 관리한다.
근거: `docs/superpowers/specs/2026-09-17-account-withdrawal-design.md` §5.3

- [ ] **후기 본문의 식별 내용** — 작성자를 익명화해도 "저는 OO동 사는데" 같은 문장이 본문에 남는다.
      따라서 익명화된 후기를 "완전한 익명정보"라고 단정할 수 없다. 본문 보존은 결정 2에서 수용됨.
- [ ] **후기 사진의 EXIF** — 업로드 시 촬영 위치·기기 정보를 제거하지 않는다.
      2026-09-21부터 계정 최종 파기 시 현재 첨부 사진은 삭제하지만 활동 중·7일 유예 중에는 남는다.
      기존 고아 파일·이미 파기된 계정의 사진·백업은 별도 점검/정리 대상이다.
- [ ] **하루 탈퇴자가 1명일 때의 날짜 절삭** — `WithdrawnEmailHash.expires_at`의 날짜 단위 절삭은
      같은 날 여러 명이 탈퇴해야 효과가 있다. 다만 `AnonymousActor`에 시각 컬럼이 없어 짝지을
      기준 자체가 없으므로 현재로서는 공격이 성립하지 않는다. 운영 후 재검토.
- [ ] **나머지 항목의 자동 파기 배치** — 탈퇴 파기는 구현했으나, 처리방침이 약속한 다른 기간
      (최근 본 장소 6개월, 동행 모집 글 30일 등)의 자동 삭제는 아직 없다. 기간을 적어 두고 삭제
      코드가 없으면 처리방침이 사실과 달라진다.
