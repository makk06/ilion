...
> **2026-09-21 변경:** 이 문서는 최초 설계 이력입니다. 사진 보존 결정은 변경되어, 기존 7일 철회 기간이 지난 계정의 최종 파기 시 현재 첨부 사진 파일과 참조를 삭제합니다. 글·평점은 보존합니다. 파일 삭제 실패는 다음 배치에서 재시도하며 저장소 삭제는 DB 롤백으로 복구되지 않습니다. [최신 준비 상태](../../legal/README.md)를 우선합니다.
