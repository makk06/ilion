...
**배포 적용:** 이 주소는 공개 시안 변경이 포함된 새 이미지 적용 후 열린다. 최초 배포 커밋 `67d37c03c017`에서는 404였으며 문서 변경만으로 운영 화면이 바뀌지는 않는다. 로컬 시연은 [백엔드 실행 안내](./README.md)에 따라 `http://127.0.0.1:8000/test/backend/`를 사용한다.
