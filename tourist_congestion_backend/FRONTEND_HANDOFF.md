...
| 내 프로필 조회/수정·비밀번호 재설정·이메일 인증 | 현재 공개 라우트 없음 |
| **회원 탈퇴·탈퇴 철회** | 구현 (2026-09-18). 아래 「회원 탈퇴 API」 참조 |
...
## 회원 탈퇴 API (2026-09-18 추가)

탈퇴는 **즉시 파기가 아니라 7일 유예** 방식이다. 유예 동안은 로그인만 막히고 데이터는 그대로
있으며, 7일이 지나면 배치가 파기한다.

### `POST /api/me/withdraw` — 탈퇴 신청

인증 필요. 본인 확인 수단은 가입 경로에 따라 다르다.

| 필드 | 필수 | 설명 |
|---|---|---|
| `password` | 이메일 가입자 | 비밀번호 재확인 |
| `id_token` | 구글 가입자 | 구글 재인증 토큰 |
| `reason_code` | 선택 | 아래 목록 중 하나. **자유 입력 불가** |

`reason_code` 허용값: `no_longer_needed`, `few_places`, `inaccurate_crowd`, `privacy_concern`,
`switched_service`, `etc`

```json
{ "success": true, "data": { "purge_at": "2026-09-25T10:00:00+00:00" },
  "message": "탈퇴가 접수되었습니다. 7일 이내에 로그인하시면 취소할 수 있습니다." }
```

본인 확인 실패 시 `401`. 성공하면 **모든 기기의 refresh token이 즉시 폐기**되므로 앱은 로그아웃
상태로 전환해야 한다.

### `POST /api/auth/login` — 유예 중 계정의 로그인

유예 중 계정은 로그인이 막힌다. 다만 **자격 증명이 맞으면** 일반 401 대신 `403`에 복구 안내가 온다.

```json
{ "success": false,
  "data": { "code": "withdrawal_pending", "purge_at": "2026-09-25T10:00:00+00:00" },
  "message": "탈퇴 예정 계정입니다. 복구할 수 있습니다." }
```

앱은 이 응답을 받으면 "탈퇴 예정입니다. 복구하시겠습니까?"를 띄우고, 동의하면 아래 철회
엔드포인트를 호출한다. **비밀번호가 틀리면 평범한 401이 온다** — 탈퇴 사실을 남에게 노출하지
않기 위한 것이므로, 앱도 401을 특별하게 처리하지 말 것.

### `POST /api/me/withdraw/cancel` — 탈퇴 철회

**인증 불필요.** 유예 중에는 로그인이 막혀 토큰을 받을 수 없으므로 자격 증명을 직접 보낸다.

| 필드 | 필수 | 설명 |
|---|---|---|
| `email` | 예 | |
| `password` 또는 `id_token` | 예 | 가입 경로에 맞는 것 |

성공하면 로그인과 동일하게 `access_token`·`refresh_token`을 돌려준다. 파기 기한이 이미 지난
계정은 `409`이며 복구할 수 없다.

### 통합 프런트 동작 (2026-09-20)

- 탈퇴 버튼은 별도 안내 화면에서 현재 비밀번호를 확인하고 위 POST 계약을 사용한다.
  PR #16의 과거 `DELETE /api/auth/withdraw` 경로는 배포 계약에 포함하지 않는다.
- 성공 응답을 받은 뒤에만 로컬 세션을 삭제한다. 실패 시 화면에 오류를 표시하고 다시 시도할 수 있다.
- 로그인 시 `withdrawal_pending`의 `purge_at`을 표시하고, 사용자 동의가 있어야 철회한다.
...
- 후기/사진의 자유 입력은 계정 연결 제거만으로 개인정보가 없어짐을 보장하지 않는다.
  앱은 탈퇴 전에 개인정보가 든 콘텐츠를 직접 삭제하도록 안내한다.

### 재가입 제한

탈퇴 파기 후 **30일간 같은 이메일로 가입할 수 없다.** 거부 응답은 `400`이며 메시지는
"지금은 이 이메일로 가입할 수 없습니다."다. 탈퇴 사실을 밝히지 않으므로 앱도 사유를 추측해
안내하지 말 것.

### ⚠ 리뷰 응답 계약 변경 (기존 화면 수정 필요)

탈퇴자의 후기는 지워지지 않고 **작성자만 익명화**되어 남는다. 따라서 후기 응답의 작성자 필드가
비어 있을 수 있다.

| 필드 | 변경 |
|---|---|
| `author_id` | **`null`이 될 수 있다** (이전에는 항상 정수) |
| `author_nickname` | `author_id`가 `null`이면 `"탈퇴한 사용자"` |
| `is_mine` | 익명화된 후기는 항상 `false` |

프로필 이동 등 `author_id`를 쓰는 화면은 **`null` 분기를 넣어야 한다.** 넣지 않으면 탈퇴자 후기를
탭했을 때 오류가 난다.

동행 모집(`companion`) 응답은 **바뀌지 않는다.** 모집 글과 참여 이력은 탈퇴 시 삭제되므로
작성자가 비는 경우가 없다.
