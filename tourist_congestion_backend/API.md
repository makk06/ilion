...
클라이언트가 지도 SDK를 초기화할 때 필요한 공개 설정을 반환합니다. 인증은
필요하지 않으며 응답은 저장하지 않도록 `Cache-Control: no-store`가 설정됩니다.
...
서버에 `VWORLD_API_KEY`가 설정되지 않았으면 HTTP 503을 반환합니다. 이 값은
클라이언트에 공개되는 키이므로 운영에서는 VWorld 콘솔에서 허용 도메인이나 앱을
서비스 범위로 제한해야 합니다.
