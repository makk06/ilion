...
class BackendTestDashboardTests(TestCase):
    ...
    def test_post_and_virtual_scenario_unavailable_without_debug(self):
        response = self.client.post(self.url, {'scenario': 'weather'})
    ...
    @override_settings(DEBUG=False)
    def test_unavailable_without_debug(self):
        response = self.client.get(self.url)
