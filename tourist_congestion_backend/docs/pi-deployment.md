...
배포 중단 시 Django가 `users.0006`만 커밋하고 `users.0007`은 남길 수 있다. 같은 승인
대상으로 재시도할 때는 이 정확한 잔여 계획(`users.0007` 한 건)도 복구 경로로 허용한다.
재시도 전 현재 부분 적용 DB를 다시 백업하고 무결성을 검사하며, 다른 migration 조합은
여전히 기동을 거부한다.
