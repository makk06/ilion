...
<http://127.0.0.1:8000/test/backend/>를 엽니다. 이 페이지는 **임시 테스트베드**로
`DJANGO_DEBUG=true`일 때만 열립니다. 일곱 도시/자유 위치 실제 추천, 같은 장소의
...
외부 API 호출, 수집 작업 등록, DB 변경을 하지 않습니다. 수집 작업·예산 표시는
DB에 기록된 범위이므로 운영 모니터링이나 서비스 품질 지표로 해석하지 않습니다.
